...
import wrs.neuro._kinematics.jlchain as rkjlc
import wrs.robot_sim._kinematics.collision_checker as cc
import torch


class ManipulatorInterfaceNeuro(object):

    def __init__(self, pos=torch.zeros(3), rotmat=torch.eye(3), home_conf=torch.zeros(6), name='manipulator', enable_cc=False):
        self.name = name
        # jlc
        ...
        self._loc_tcp_pos = torch.zeros(3, dtype=float)
        self._loc_tcp_rotmat = torch.eye(3, dtype=float)
        self._is_gl_tcp_delayed = True
        self._gl_tcp_pos = torch.zeros(3, dtype=float)
        self._gl_tcp_rotmat = torch.eye(3, dtype=float)
        # TODO self.ooflange = []
        # collision detection
        if enable_cc:
    ...
    def jacobian(self, jnt_values=None):
        if jnt_values is None:
            j_mat = torch.zeros((6, self.jlc.n_dof))
            for i in range(self.jlc.flange_jnt_id + 1):
                if self.jlc.jnts[i].type == rkjlc.const.JntType.REVOLUTE:
                    ...
                    j_mat[:3, i] = torch.cross(self.jlc.jnts[i].gl_motion_ax, j2t_vec)
            ...
            jnt_pos = torch.zeros((self.jlc.n_dof, 3))
            jnt_motion_ax = torch.zeros((self.jlc.n_dof, 3))
            for i in range(self.jlc.flange_jnt_id + 1):
            ...
            for i in range(self.jlc.flange_jnt_id + 1):
                if self.jlc.jnts[i].type == rkjlc.const.JntType.REVOLUTE:
                    ...
                    j_mat[:3, i] = torch.cross(jnt_motion_ax[i, :], j2t_vec)
    ...
    def manipulability_val(self):
        j_mat = self.jacobian()
        return torch.sqrt(torch.linalg.det(j_mat @ j_mat.T))

    def manipulability_mat(self):
        j_mat = self.jacobian()
        # linear ellipsoid
        linear_j_dot_jt = j_mat[:3, :] @ j_mat[:3, :].T
        eig_values, eig_vecs = torch.linalg.eig(linear_j_dot_jt)
        linear_ellipsoid_mat = torch.eye(3)
        linear_ellipsoid_mat[:, 0] = torch.sqrt(eig_values[0]) * eig_vecs[:, 0]
        linear_ellipsoid_mat[:, 1] = torch.sqrt(eig_values[1]) * eig_vecs[:, 1]
        linear_ellipsoid_mat[:, 2] = torch.sqrt(eig_values[2]) * eig_vecs[:, 2]
        # angular ellipsoid
        angular_j_dot_jt = j_mat[3:, :] @ j_mat[3:, :].T
        eig_values, eig_vecs = torch.linalg.eig(angular_j_dot_jt)
        angular_ellipsoid_mat = torch.eye(3)
        angular_ellipsoid_mat[:, 0] = torch.sqrt(eig_values[0]) * eig_vecs[:, 0]
        angular_ellipsoid_mat[:, 1] = torch.sqrt(eig_values[1]) * eig_vecs[:, 1]
        angular_ellipsoid_mat[:, 2] = torch.sqrt(eig_values[2]) * eig_vecs[:, 2]
        return (linear_ellipsoid_mat, angular_ellipsoid_mat)

    def cvt_pose_in_tcp_to_gl(self, loc_pos=torch.zeros(3), loc_rotmat=torch.eye(3)):
