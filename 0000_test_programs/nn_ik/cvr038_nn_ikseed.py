...
if __name__ == '__main__':
    ...
    if backbone == 'IKMLPNet':
        model = IKMLPNet(input_dim=model_in.shape[1], output_dim=model_out.shape[1]).to(device).float()
    elif backbone == 'IKMLPScaleNet':
        model = IKMLPScaleNet(input_dim=model_in.shape[1], output_dim=model_out.shape[1], robot=robot).to(device).float()
    elif backbone == 'IKLSTMNet':
        model = IKLSTMNet(input_dim=model_in.shape[1], output_dim=model_out.shape[1], hidden_dim=64, num_layers=2, robot=robot).to(device).float()
    else:
