# =========================================================
# Rev2 CVT vs Random Uniformity Analysis
# =========================================================
...
# =========================================================
# Rev2 CVT KDTree Build Time Analysis
# =========================================================
import os, json, numpy as np, matplotlib.pyplot as plt
from scipy.spatial import cKDTree
...
def main():
    # ① 扫描 artifacts 下所有 npy 文件
    npy_files = sorted([f for f in os.listdir(ARTIFACT_DIR) if f.endswith("r0.npy")])
    if not npy_files:
        print("❌ No .npy files found in artifacts/")
        return
    ...
    for f in npy_files:
        ...
        try:
            data = np.load(fpath)
            nearest, stats = analyze_uniformity(data)
            stats["file"] = f
            summary.append(stats)

            # ② 绘制并保存 NN 距离分布图
            plt.figure(figsize=(6, 4))
            plt.hist(nearest, bins=50, color='steelblue', alpha=0.7)
            plt.xlabel("Nearest Neighbor Distance")
            plt.ylabel("Count")
            plt.title(f"NN Distance Distribution\n{f}")
            plt.tight_layout()
            plt.savefig(os.path.join(OUTPUT_DIR, f"{f.replace('.npy','')}_nn_hist.png"), dpi=200)
            plt.close()

            # ③ 输出控制台结果
            print(f"{f}: mean={stats['mean_nn']:.2f}, std={stats['std_nn']:.2f}, cv={stats['cv_nn']:.2f}")
    ...
    print(f"\n📊 Saved summary JSON → {SUMMARY_JSON}")
    print(f"✅ Analyzed {len(summary)} .npy files.")
