...
def normalize_dataset(dataset):
    ...
    jnt_values, tgt_pos, tgt_rot = dataset['jnt_values'], dataset['tgt_pos'], dataset['tgt_rotmat']
    jnt_ranges = robot.jnt_ranges[:,1] - robot.jnt_ranges[:,0]
    normed_jnt_values = 2 * (jnt_values - robot.jnt_ranges[:,0]) / jnt_ranges - 1

    print('Normalizing dataset finished')
    return normed_jnt_values, tgt_pos, tgt_rot
...
if __name__ == '__main__':
    ...
    jnt_values, tgt_pos, tgt_rotmat  = normalize_dataset(dataset)
    ...
    if mode == 'train':
        ...
        rootpath = f'{TimeCode}_{backbone}_{dataset_size}_normed'
