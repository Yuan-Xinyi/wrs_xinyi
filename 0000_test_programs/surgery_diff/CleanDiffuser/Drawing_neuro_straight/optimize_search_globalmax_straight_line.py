...
table = mcm.gen_box(xyz_lengths=[2.0, 2.0, 0.03], pos=[0.0, 0, -0.014], rgb=[0.6, 0.4, 0.2])
table.attach_to(base)
paper = mcm.gen_box(xyz_lengths=[1.8, 1.8, 0.002], pos=[0.0, 0, 0.001], rgb=[1, 1, 1])
...
if __name__ == "__main__":
    ...
    # 配置参数
    # ============================================================
    num_xc_samples = 1024         # 遍历的中心点总数
    batch_size_per_xc = 512       # 每个 xc 采样的方向数
    epsilon = 0.01                # 二分精度
    
    global_best_L = 0.1           # 全局已知的最长长度（初始下界）
    global_best_q = None          # 对应最长轨迹的关节角
    global_best_pos = None        # 对应最长轨迹的位置
    global_best_xc = None         # 对应最长的中心点

    # 预先采样一批中心点
    candidate_xcs, _, _ = sampler.sample_tasks(batch_size=num_xc_samples, L_range=(0.1, 0.1))
    for xcs in candidate_xcs:
        mcm.gen_sphere(pos=xcs.cpu().numpy(), radius=0.0025, rgb=[1,0,0]).attach_to(base)
    
    print(f"[INFO] Sampled {num_xc_samples} candidate centers.")

    for i, xc_single in enumerate(candidate_xcs):
        print(f"\n\n{'='*60}")
        print(f"[XC {i+1}/{num_xc_samples}] Testing Center: {xc_single.cpu().numpy()}")
        print(f"{'='*60}")

        # 设置当前 xc 的搜索范围，L_low 直接继承 global_best_L
        L_low = global_best_L
        L_high = 1.4  # 机械臂展弦极限

        local_best_q = None
        local_best_pos = None
        current_xc_success = False

        # --- 新增：准入测试 (Entrance Test) ---
        # 逻辑：如果该 xc 在当前全球最优长度下都跑不通，则没必要进行后续二分尝试
        print(f"  [Entrance Test] Checking if L_low = {L_low:.3f} is feasible...")
        xc_batch_test = xc_single.repeat(batch_size_per_xc, 1)
        theta_test = torch.rand(batch_size_per_xc, device=device) * 2 * np.pi
        ds_test = torch.stack([torch.cos(theta_test), torch.sin(theta_test), torch.zeros_like(theta_test)], dim=1)
        Ls_test = torch.full((batch_size_per_xc,), L_low, device=device)

        pos_paths_test = sample_line_batch(xc_batch_test, ds_test, Ls_test, num_points=32)
        q_opt_test = optimize_path_batch(pos_paths_test, cc=None, steps=50)
        success_flag_test, _ = evaluate_trajectory_batch(cc_model, q_opt_test, pos_paths_test)

        if not success_flag_test.any():
            print(f"  [Skip] xc_{i+1} cannot reach current Global Best L={L_low:.3f}. Skipping...")
            continue
        else:
            # 准入成功，作为该 xc 的保底结果
            current_xc_success = True
            first_idx = torch.nonzero(success_flag_test)[0, 0]
            local_best_q = q_opt_test[first_idx]
            local_best_pos = pos_paths_test[first_idx]
            print(f"  [Entrance Test] Passed. Starting binary search to exceed {L_low:.3f}...")
        ...
        while (L_high - L_low) > epsilon:
            ...
            # 采样固定 xc 和固定 L_mid 的多个方向
            xc_batch = xc_single.repeat(batch_size_per_xc, 1)
            theta = torch.rand(batch_size_per_xc, device=device) * 2 * np.pi
            ds = torch.stack([torch.cos(theta), torch.sin(theta), torch.zeros_like(theta)], dim=1)
            Ls = torch.full((batch_size_per_xc,), L_mid, device=device)

            pos_paths = sample_line_batch(xc_batch, ds, Ls, num_points=32)

            # 优化与评估
            q_optimized_batch = optimize_path_batch(pos_paths, cc=None, steps=50)
            success_flag, _ = evaluate_trajectory_batch(cc_model, q_optimized_batch, pos_paths)
            
            # 实时显示当前进度与成功率
            s_rate = success_flag.sum().item() / batch_size_per_xc
            print(f"  [Binary {iter_id}] Try L = {L_mid:.3f} Success rate = {s_rate:.2f} (Global Best: {global_best_L:.3f})")
            ...
            if success_flag.any():
                ...
                # 记录该 xc 下目前最好的解
                first_idx = torch.nonzero(success_flag)[0, 0]
                local_best_q = q_optimized_batch[first_idx]
                local_best_pos = pos_paths[first_idx]
            else:
                L_high = L_mid
        
        # 更新全局最优
        if current_xc_success and L_low > global_best_L:
            print(f"✨ NEW GLOBAL RECORD: L = {L_low:.3f} at XC {i+1}")
            global_best_L = L_low
            global_best_q = local_best_q
            global_best_pos = local_best_pos
            global_best_xc = xc_single

    # ============================================================
    # 最终结果可视化
    # ============================================================
    print("\n" + "#"*40)
    print(f"FINAL RESULT: Max L = {global_best_L:.4f}")
    print(f"Best Center XC: {global_best_xc.cpu().numpy() if global_best_xc is not None else 'None'}")
    print("#"*40)

    if global_best_q is not None:
        # 画出那根最长的蓝线
        mgm.gen_stick(global_best_pos[0].cpu().numpy(),
                      global_best_pos[-1].cpu().numpy(),
                      radius=0.003,
                      rgb=[0, 0, 1]).attach_to(base)
        
        # 动画演示
        q_vis = global_best_q.reshape(-1, 6).cpu().numpy()
