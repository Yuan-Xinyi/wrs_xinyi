...
from diffusion import DEFAULT_H5_PATH, DEFAULT_RUN_NAME, DEFAULT_WORKDIR
from diffusion_sample import JacobianCorrection, load_model, normalize_direction
from diffusion import sample_q_length_from_condition
from trajectory_generation.xarmlite6_nullspace_straight_demo import TrackerConfig, NullspaceStraightTracker
from wrs.robot_sim.robots.xarmlite6_wg.xarm6_drill import XArmLite6Miller


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Randomly sample one GT token from HDF5, run diffusion inpainting, compare to GT, and visualize both trajectories.')
...
def evaluate_predictions(robot, gt, q_preds, pred_lengths, correction, tracker):
    results = []
        ...
        results.append(
            {
                ...
                'q_pred_to_corr_l2': float(np.linalg.norm(q_corr - q_pred)),
                'pos_err': float(pos_err),
                'q_l2_to_gt': float(np.linalg.norm(q_corr - gt['q'])),
                'predicted_length': float(pred_length),
                'predicted_length_err_to_gt': float(abs(float(pred_length) - gt['length'])),
                'track': track,
...
def choose_best_prediction(predictions: list[dict], gt_length: float) -> dict:
    return min(
        ...
        key=lambda item: (
            ...
            abs(item['predicted_length'] - gt_length),
            item['pos_err'],
            item['q_l2_to_gt'],
        ),
    )
...
def visualize_comparison(robot: XArmLite6Miller, gt: dict, gt_track, best_pred: dict, all_preds: list[dict], show_all_pred_ends: bool) -> None:
    ...
    pred_end_ideal = start_pos + direction * best_pred['predicted_length']
    ...
    robot.goto_given_conf(best_pred['q_pred'])
...
def main() -> None:
    ...
    predictions = evaluate_predictions(robot, gt, q_preds, pred_lengths, correction, tracker)
    ...
    for pred in predictions:
        print(
            f"[pred {pred['sample_idx']}] pos_err={pred['pos_err']:.6f} q_l2={pred['q_l2_to_gt']:.6f} q_pred_to_corr_l2={pred['q_pred_to_corr_l2']:.6f} "
            f"predicted_length={pred['predicted_length']:.6f} predicted_length_err={pred['predicted_length_err_to_gt']:.6f} "
    ...
    summary = {
        ...
        'best_pred': {
            ...
            'q_pred': best_pred['q_pred'].tolist(),
            'q_corr': best_pred['q_corr'].tolist(),
            'q_pred_to_corr_l2': float(best_pred['q_pred_to_corr_l2']),
            'predicted_length': float(best_pred['predicted_length']),
            'predicted_length_err_to_gt': float(best_pred['predicted_length_err_to_gt']),
