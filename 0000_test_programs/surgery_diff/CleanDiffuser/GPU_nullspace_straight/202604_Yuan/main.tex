...
\xinyi{
    \begin{tabular}{llcccccccccc}
        ...
        & Continuous Optimization & \rev{---}  & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} \\
        & Receding-horizon MPC & \rev{---}  & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} & \rev{---} \\
        \midrule
...
\begin{tablenotes}
    ...
    \item[Note 2] The two classical baselines are implemented separately but are omitted here because the full 2,000-task runs were not finalized at the time of this draft.
\end{tablenotes}
}
\end{threeparttable}
\end{table*}
...
For representative non-generative baselines, we implement one continuous-optimization baseline and one receding-horizon MPC baseline on the same long-task list. The continuous-optimization baseline directly treats the initial joint configuration $\mathbf{q}_{0}$ as the decision variable. Starting from several random feasible seeds, it first optimizes start-pose consistency with respect to the prescribed TCP position and normal, and then maximizes a differentiable finite-horizon rollout objective that rewards forward progress while penalizing position-tracking error, orientation violation, joint-boundary violation, low manipulability, collision cost, and excessive control magnitude. The receding-horizon MPC baseline instead optimizes a short control sequence online. At each step, it solves a finite-horizon control problem under the same differentiable rollout objective, executes only the first control increment, and replans from the updated state until termination. Both baselines log per-case timing and outcome statistics to JSONL files, enabling direct comparison between test-time generation and classical online optimization/control strategies.
