...
class Agent(nn.Module):
    ...
    def __init__(self, envs):
        ...
        # Action log std (VERY important!!)
        # smaller std -> smooth actions
...
# ---------------------------------------------------------------------
# 4. RobotsEnv (FK-only, stable)
# ---------------------------------------------------------------------

class Robots:
    """Just a wrapper to access FK in batch."""
...
class RobotsEnv:
    """FK-only multi-env for Δq reaching task"""
    def __init__(self, num_envs, device):
        ...
        self.goal_half = 0.05   # small region for curriculum
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations+1):
        ...
        for step in range(args.num_steps):
            ...
            # next_done = torch.tensor(np.logical_or(term, trunc), dtype=torch.float32).to(device)
            ...
            if "final_info" in info:
                for f in info["final_info"]:
                    if f is not None:
                        writer.add_scalar("episodic_return", f["episode"]["r"], global_step)
                        writer.add_scalar("episodic_length", f["episode"]["l"], global_step)
                        print(f"[step {global_step}] return={f['episode']['r']}")
        ...
        print(f"SPS: {global_step/(time.time()-start_time):.1f}")
