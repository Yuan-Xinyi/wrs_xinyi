...
# }
robot_constructors = {
    'fr3': lambda: franka.FrankaResearch3(enable_cc=True)
}
...
for name, ctor in robot_constructors.items():
    ...
    for jnt in scaled:
        ...
        workspace_list.append(pos)
    
    '''workspace statics'''
    workspace = np.array(workspace_list)
    print(f"[{name}] workspace shape:", workspace.shape)
    print(f"[{name}] workspace range:\n", workspace.min(axis=0), "\n", workspace.max(axis=0))
