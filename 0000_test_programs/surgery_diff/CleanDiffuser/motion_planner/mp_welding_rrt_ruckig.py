...
def get_ee_positions_from_jnt_list(jnt_list, robot):
    """
    给定关节角度列表，返回末端执行器的位置列表。
    
    参数:
        jnt_list (List[np.ndarray]): 每个元素是 shape=(7,) 的关节角向量
        robot: 机器人对象，需要实现 robot.fk(jnt_values) -> (pos, rotmat)
    
    返回:
        List[np.ndarray]: 每个元素是 shape=(3,) 的末端位置 XYZ 
    """
    ee_positions = []
    for jnt in jnt_list:
        if jnt is not None:
            pos, _ = robot.fk(jnt_values=jnt)
            ee_positions.append(pos)
    return ee_positions

def plot_ee_xyz_components_aligned(ee_pos_list, labels=None):
    """
    将每条末端轨迹的 x, y, z 分量分别绘制成三个子图，时间归一化（首尾对齐但不插值）。

    参数:
        ee_pos_list: List of (n, 3) arrays/lists 表示多条末端轨迹
        labels: List of labels for each trajectory
    """
    if labels is None:
        labels = [f"Trajectory {i+1}" for i in range(len(ee_pos_list))]

    fig, axes = plt.subplots(3, 1, figsize=(10, 8), sharex=True)
    component_names = ['X', 'Y', 'Z']

    for traj, label in zip(ee_pos_list, labels):
        traj = np.array(traj)  # shape (n, 3)
        t = np.linspace(0, 1, len(traj))  # 时间归一化
        for i in range(3):
            axes[i].plot(t, traj[:, i], label=label)

    for i in range(3):
        axes[i].set_ylabel(component_names[i])
        axes[i].grid(True)
        axes[i].legend()

    axes[-1].set_xlabel("Normalized Time")
    fig.suptitle("End-Effector Trajectories (X, Y, Z Components Aligned)", fontsize=14)
...
if __name__ == '__main__':
    ...
    ee_pos_list = []
    for waypoint_num in [5, 30, 50, 70, 100]:
        ...
        ee_pos_list.append(get_ee_positions_from_jnt_list(jnt_path, robot))
        
        from pathlib import Path
        from plotter import Plotter
        pdf_path = os.path.join('/home/lqin/zarr_datasets', f'traj_waypos{waypoint_num}.pdf')
        Plotter.plot_trajectory(pdf_path, otg, inp, out_list, plot_jerk=False)
    plot_ee_xyz_components_aligned(ee_pos_list, labels=[f'Waypoint {n}' for n in [5, 30, 50, 70, 100]])
