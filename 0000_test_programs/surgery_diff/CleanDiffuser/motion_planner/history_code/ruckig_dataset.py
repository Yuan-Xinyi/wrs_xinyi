...
class StraightLineDataset(BaseDataset):
    def __init__(self,
            zarr_path,
            obs_keys=[], 
            horizon=1,
    ...
    def _sample_to_data(self, sample):
        # ['jnt_pos', 'position', 'rotation', 'axis']
        init_jnt_pos = sample['jnt_pos'][0]
        position = sample['position']
        
        start_position, end_position = position[0], position[-1]
        if self.normalizer:
            init_jnt_pos = self.normalizer['obs']['jnt_pos'].normalize(init_jnt_pos)
