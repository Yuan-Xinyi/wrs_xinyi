...
class PosVelAccPlanningDataset(BaseDataset):
    def __init__(self,
            zarr_path,
            obs_keys=[], 
            horizon=1,
    ...
    def _sample_to_data(self, sample):
        ...
        '''condition'''
        condition = np.concatenate([sample['jnt_pos'][0], sample['jnt_vel'][0], sample['jnt_acc'][0],
                                    sample['jnt_pos'][-1], sample['jnt_vel'][-1], sample['jnt_acc'][-1]], axis=-1)
