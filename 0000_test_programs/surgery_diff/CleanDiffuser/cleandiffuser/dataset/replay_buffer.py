...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        for key, value in root['data'].items():
            # assert (value.shape[0] == int(root['meta']['episode_ends'][-1]))
            if not (value.shape[0] == int(root['meta']['episode_ends'][-1])):
                value = value[:int(root['meta']['episode_ends'][-1])]
        self.root = root
    ...
    def copy_from_store(cls, src_store, store=None, keys=None,
        ...
        toc = time.time()
        print(f"ReplayBuffer.copy_from_store: {toc - tic:.2f}s")
        return buffer
